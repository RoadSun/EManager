...
//  EManager
//
//  Created by EX DOLL on 2019/2/27.
//  Copyright © 2019 EX DOLL. All rights reserved.
//

import UIKit

class SColorView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        //创建进度文本标签
        label = UILabel(frame:CGRect(x:100, y:200, width:bounds.width,
...
//        label.text = "0℃"
        ...
        path.addLine(to: CGPoint(x: self.w, y: 0))
        path.addLine(to: CGPoint(x: self.w, y: self.h))
        path.addLine(to: CGPoint(x: 0, y: self.h))
        path.close()
        //绘制进度条背景轨道
        ...
        setPercent(percent: 100, animated: true)
    }
